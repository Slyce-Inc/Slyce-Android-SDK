...
public class AsyncSSLSocketWrapper implements com.android.slyce.async.wrapper.AsyncSocketWrapper, AsyncSSLSocket {
    ...
    AsyncSocket mSocket;
    com.android.slyce.async.BufferedDataSink mSink;
    boolean mUnwrapping;
    SSLEngine engine;
    ...
    com.android.slyce.async.callback.WritableCallback mWriteableCallback;
    ...
    public static void handshake(AsyncSocket socket,
        ...
        socket.setClosedCallback(new com.android.slyce.async.callback.CompletedCallback() {
            @Override
    ...
    private AsyncSSLSocketWrapper(AsyncSocket socket,
        ...
        mSink = new com.android.slyce.async.BufferedDataSink(socket);
        mSink.setWriteableCallback(new com.android.slyce.async.callback.WritableCallback() {
            @Override
        ...
        mSocket.setEndCallback(new com.android.slyce.async.callback.CompletedCallback() {
            @Override
            public void onCompleted(Exception ex) {
                if (mEnded)
                    return;
    ...
    final com.android.slyce.async.callback.DataCallback dataCallback = new com.android.slyce.async.callback.DataCallback() {
        final com.android.slyce.async.util.Allocator allocator = new com.android.slyce.async.util.Allocator().setMinAlloc(8192);
        final ByteBufferList buffered = new ByteBufferList();

        @Override
        public void onDataAvailable(com.android.slyce.async.DataEmitter emitter, ByteBufferList bb) {
            if (mUnwrapping)
                return;
            try {
                mUnwrapping = true;
    ...
    public void onDataAvailable() {
        com.android.slyce.async.Util.emitAllData(this, pending);
    ...
    private void handleHandshakeStatus(HandshakeStatus status) {
        ...
        try {
            if (!finishedHandshake && (engine.getHandshakeStatus() == HandshakeStatus.NOT_HANDSHAKING || engine.getHandshakeStatus() == HandshakeStatus.FINISHED)) {
                if (clientMode) {
                    ...
                    if (!trusted) {
                        com.android.slyce.async.AsyncSSLException e = new com.android.slyce.async.AsyncSSLException(peerUnverifiedCause);
                        report(e);
                        if (!e.getIgnore())
                            throw e;
                    }
        ...
        catch (com.android.slyce.async.AsyncSSLException ex) {
            report(ex);
        }
    }
    ...
    private void report(Exception e) {
        ...
        if (hs != null) {
            handshakeCallback = null;
            mSocket.setDataCallback(new com.android.slyce.async.callback.DataCallback.NullDataCallback());
            mSocket.end();
            mSocket.close();
        ...
        com.android.slyce.async.callback.CompletedCallback cb = getEndCallback();
        if (cb != null)
            cb.onCompleted(e);
    }
    ...
    @Override
    public com.android.slyce.async.DataEmitter getDataEmitter() {
        return mSocket;
    }
