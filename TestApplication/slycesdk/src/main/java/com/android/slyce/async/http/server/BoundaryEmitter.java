...
public class BoundaryEmitter extends com.android.slyce.async.FilteredDataEmitter {
...
//        System.out.println("state: " + state);
        ...
        if (state > 0) {
            ByteBuffer b = com.android.slyce.async.ByteBufferList.obtain(boundary.length);
        ...
        for (int i = 0; i < buf.length; i++) {
            if (state >= 0) {
                ...
                if (buf[i] == '\r') {
                    ...
                    if (last != 0 || len != 0) {
                        ByteBuffer b = com.android.slyce.async.ByteBufferList.obtain(len).put(buf, last, len);
                        b.flip();
                        com.android.slyce.async.ByteBufferList list = new com.android.slyce.async.ByteBufferList();
                        list.add(b);
...
//                    System.out.println("bstart");
                    ...
                    report(new com.android.slyce.async.http.server.MimeEncodingException("Invalid multipart/form-data. Expected \r or -"));
                    return;
                }
            }
            else if (state == -2) {
                if (buf[i] == '-') {
                    ...
                    report(new com.android.slyce.async.http.server.MimeEncodingException("Invalid multipart/form-data. Expected -"));
                    return;
                }
            }
            else if (state == -3) {
                if (buf[i] == '\r') {
                    state = -4;
                    ByteBuffer b = com.android.slyce.async.ByteBufferList.obtain(i - last - boundary.length - 2).put(buf, last, i - last - boundary.length - 2);
                    b.flip();
                    com.android.slyce.async.ByteBufferList list = new com.android.slyce.async.ByteBufferList();
                    list.add(b);
                    super.onDataAvailable(this, list);
//                    System.out.println("bend");
                    ...
                    report(new com.android.slyce.async.http.server.MimeEncodingException("Invalid multipart/form-data. Expected \r"));
                    return;
                }
            }
            else if (state == -4) {
                ...
                assert false;
                report(new com.android.slyce.async.http.server.MimeEncodingException("Invalid multipart/form-data. Unknown state?"));
            }
        }
...
//            System.out.println(state);
            ...
            ByteBuffer b = com.android.slyce.async.ByteBufferList.obtain(buf.length - last - keep).put(buf, last, buf.length - last - keep);
            b.flip();
            com.android.slyce.async.ByteBufferList list = new com.android.slyce.async.ByteBufferList();
            list.add(b);
            super.onDataAvailable(this, list);
