...
interface FrameWriter extends Closeable {
   ...
   */
  void pushPromise(int streamId, int promisedStreamId, List<com.android.slyce.async.http.spdy.Header> requestHeaders)
      throws IOException;

  /** SPDY/3 only. */
  void synStream(boolean outFinished, boolean inFinished, int streamId, int associatedStreamId,
                 List<com.android.slyce.async.http.spdy.Header> headerBlock) throws IOException;
  void synReply(boolean outFinished, int streamId, List<com.android.slyce.async.http.spdy.Header> headerBlock)
      throws IOException;
  void headers(int streamId, List<com.android.slyce.async.http.spdy.Header> headerBlock) throws IOException;
  void rstStream(int streamId, com.android.slyce.async.http.spdy.ErrorCode errorCode) throws IOException;
   ...
   */
  void data(boolean outFinished, int streamId, com.android.slyce.async.ByteBufferList source) throws IOException;
   ...
   */
  void goAway(int lastGoodStreamId, com.android.slyce.async.http.spdy.ErrorCode errorCode, byte[] debugData) throws IOException;
